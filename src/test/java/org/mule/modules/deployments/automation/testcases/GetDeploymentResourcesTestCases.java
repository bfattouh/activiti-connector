...
public class GetDeploymentResourcesTestCases extends FunctionalMunitSuite {
	...
	private static final String DEPLOYMENT_RESOURCES_JSON_PAYLOAD = "["
			...
			+ "\"mediaType\":\"image/png\"," + "\"type\":\"resource\"" + "}"
			+ "]";
	...
	public void testJson() throws Exception {
		...
		List<DeploymentResource> deploymentResources = DeploymentResource
				.unMarshalJSON(clazz, DEPLOYMENT_RESOURCES_JSON_PAYLOAD);
		assertEquals(2, deploymentResources.size());
		DeploymentResource deploymentResource1 = deploymentResources.get(0);
		assertEquals("diagrams/my-process.bpmn20.xml",
				deploymentResource1.getId());
		assertEquals(
		...
		String payloadResult = DeploymentResource.marshalJSON(clazz,
				deploymentResources);
	...
	public void testGetDeploymentResources() throws Exception {
		...
		assertEquals(
				"http://localhost:8080/activiti-rest/service/repository/deployments/"
						+ deployment.getId()
						+ "/resources/accountcreation.bpmn20.xml",
				deploymentResource.getUrl());
